<template>
    <v-app>
            ...
            <v-form>
                <v-container>
                    <v-layout justify-end="true">
                        <v-flex xs6>
                            <v-combobox v-model="selectGender" :items="itemsgender" :pagination.sync="pagination" label="Select your gender"></v-combobox>
                        </v-flex>
                        <v-flex xs6>
                            <v-combobox v-model="selectAge" :items="itemsAge" label="Select your age"></v-combobox>
                        </v-flex>
      ...
      border: 2px solid #73AD21;overflow-y: scroll ;overflow-x: scroll">
                <template>
              <v-data-table :headers="headers" :items="answers" class="TFtable">
...
export default {
    ...
    methods: {
        async submit() {
            ...
            await axios
                .get(
                    encodeURI(
                        "http://localhost:3000/recommendFood?ages=" +
                        this.selectAge +
                        "&gender=" +
                        this.selectGender
                    )
                    ...
                    result => {
                        ...
                        for (i = 0; i < jsarr.length; i++) {
                            var vals =
                                ...
                                jsarr[i].foodgroup +
                                " group " +
                                " based on following instruction:\n" +
                                jsarr[i].directional;
                            this.answers[i].name = vals;
                        }
                    },
                    error => {
                        console.error(error);
    ...
    mounted() {
        axios({
            method: "GET",
            url: "http://localhost:3000/recommendFood?ages=14%20to%2018&gender=Male"
        }).then(
            result => {
